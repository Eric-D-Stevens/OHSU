...
if __name__ == "__main__":
	...
	def get_country(line):
		fail = line
		try:
			line = line.split('\t')[3] # only look at first column
		except:
	...
	def get_pop():
		pop = {}
		with open('world-population.csv') as f:
			line = f.readline()
			line = f.readline()
			while line:
				row = line.split(';')
				pop[row[1]] = float(row[2].strip())
				line = f.readline() 
		return pop
	...
	with open('country_count2.pkl', 'w') as f:
