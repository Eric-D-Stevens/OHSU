...
def transform_tf_idf(X_train_counts, X_dev_counts, X_test_counts):
    ...
    """
    vocab=list(set(X_train_counts.keys()))

    count_vec = CountVectorizer(vocabulary=vocab)
    X_train = count_vec.transform_tf_idf(X_train_counts)
    X_dev = count_vec.transform_tf_idf(X_dev_counts)
    X_test = count_vec.transform_tf_idf(X_test_counts)
